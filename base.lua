...
local multicharacter_active_identifiers = {}
local multicharacter_source_identifiers = {}

local function isIdentifierAllowed(identifier)
  return (not config.ignore_ip_identifier or string.find(identifier, "ip:", 1, true) == nil) and
         (not config.ignore_license_identifier or string.find(identifier, "license:", 1, true) == nil) and
         (not config.ignore_xbox_identifier or string.find(identifier, "xbl:", 1, true) == nil) and
         (not config.ignore_discord_identifier or string.find(identifier, "discord:", 1, true) == nil) and
         (not config.ignore_live_identifier or string.find(identifier, "live:", 1, true) == nil)
end

local function isMulticharacterStarted()
  return GetResourceState("esx_multicharacter") == "started"
      or GetResourceState("esx_kashacters") == "started"
      or GetResourceState("pure-multicharacter") == "started"
end

function vRP.getPrimaryIdentifier(source)
  local ids = GetPlayerIdentifiers(source)

  if ids ~= nil and #ids > 0 then
    for i = 1, #ids do
      if isIdentifierAllowed(ids[i]) then
        return ids[i]
      end
    end
...
local function clearUserSession(user_id, player)
  if user_id ~= nil then
    TriggerEvent("vRP:playerLeave", user_id, player)

    local data = vRP.getUserDataTable(user_id) or {}
    vRP.setUData(user_id, "vRP:datatable", json.encode(data))

    local idkey = vRP.rusers[user_id]
    if idkey ~= nil then
      vRP.users[idkey] = nil
    end
...
local function saveCharacterIdentity(user_id, identity, cbr)
  if type(identity) ~= "table" then
    cbr()
    return
  end

  local function insertIdentity(registration, phone)
    MySQL.Async.execute([[
      INSERT INTO vrp_user_identities(user_id, registration, phone, firstname, name, age)
      VALUES(@user_id, @registration, @phone, @firstname, @name, @age)
      ...
      user_id = user_id,
      registration = registration or tostring(math.random(100000, 999999)),
      phone = phone or tostring(math.random(10000000, 99999999)),
  ...
  if vRP.generateRegistrationNumber and vRP.generatePhoneNumber then
    vRP.generateRegistrationNumber(function(registration)
      vRP.generatePhoneNumber(function(phone)
        insertIdentity(registration, phone)
      end)
    end)
  else
    insertIdentity()
...
local function initCharacterDatatable(user_id, identity, cbr)
  if type(identity) ~= "table" then
    cbr()
    return
  end
  ...
  MySQL.Async.execute("REPLACE INTO vrp_user_data(user_id,dkey,dvalue) VALUES(@user_id,@key,@value)", {
    user_id = user_id,
    key = "vRP:datatable",
    value = json.encode({
      customization = {
...
local function startUserSession(player, user_id, idkey, name, cbr)
  vRP.users[idkey] = user_id
  vRP.rusers[user_id] = idkey
  vRP.user_tables[user_id] = {}
  vRP.user_tmp_tables[user_id] = {}
  vRP.user_sources[user_id] = player
  ...
  vRP.getUData(user_id, "vRP:datatable", function(sdata)
    ...
    vRP.getLastLogin(user_id, function(last_login)
      ...
      MySQL.Async.execute("UPDATE vrp_users SET last_login = @last_login WHERE id = @user_id", {user_id = user_id, last_login = last_login_stamp})

      print("["..user_id.."] Valgte karakter og forbinder til serveren")
      TriggerEvent("vRP:playerJoin", user_id, player, name, tmpdata.last_login)

      if cbr then cbr(true, user_id) end
    end)
  end)
end

local function getOrCreateCharacterUser(character_identifier, template_user_id, identity, cbr)
  MySQL.Async.fetchAll("SELECT user_id FROM vrp_user_ids WHERE identifier = @identifier", {identifier = character_identifier}, function(rows)
    if #rows > 0 then
      cbr(tonumber(rows[1].user_id), false)
      return
    end

    local function createUser(template)
      MySQL.Async.fetchAll("SELECT MAX(id) AS id FROM vrp_users", {}, function(result)
        local next_id = 1
        if result[1] and result[1].id ~= nil then
          next_id = tonumber(result[1].id) + 1
        end

        MySQL.Async.execute("INSERT INTO vrp_users (id, whitelisted, banned) VALUES (@id, @whitelisted, @banned)", {
          id = next_id,
          whitelisted = template.whitelisted or 0,
          banned = template.banned or 0
        }, function()
          MySQL.Async.execute("INSERT INTO vrp_user_ids (identifier, user_id) VALUES(@identifier, @user_id)", {
            identifier = character_identifier,
            user_id = next_id
          }, function()
            saveCharacterIdentity(next_id, identity, function()
              initCharacterDatatable(next_id, identity, function()
                cbr(next_id, true)
              end)
            end)
          end)
    ...
    if template_user_id ~= nil then
      MySQL.Async.fetchAll("SELECT whitelisted, banned FROM vrp_users WHERE id = @user_id", {user_id = template_user_id}, function(templateRows)
        createUser(templateRows[1] or {})
      end)
    else
      createUser({})
    end
...
AddEventHandler("vRP:multicharacterLogout", function(player, cbr)
  player = tonumber(player)
  if player == nil then return end

  local user_id = vRP.getUserId(player)
  clearUserSession(user_id, player)

  if cbr then cbr(true) end
end)

AddEventHandler("vRP:multicharacterLogin", function(player, character_identifier, identity, cbr)
  player = tonumber(player)
  if player == nil or type(character_identifier) ~= "string" then
    if cbr then cbr(false, "Ugyldig karakter.") end
    return
  end

  local ids = GetPlayerIdentifiers(player)
  local idkey = ids and ids[1]

  if idkey == nil then
    if cbr then cbr(false, "Kunne ikke finde din identifier.") end
    return
  end

  local current_user_id = vRP.getUserId(player)

  getOrCreateCharacterUser(character_identifier, current_user_id, identity, function(user_id, created)
    if user_id == nil then
      if cbr then cbr(false, "Kunne ikke oprette karakteren.") end
      return
    end
    ...
    vRP.getUserData(user_id, function(userdata)
      if not userdata then
        if cbr then cbr(false, "Kunne ikke hente karakterdata.") end
        return
      end

      if userdata.banned then
        local reason = userdata.ban_reason or "Ingen grund sat"
        if cbr then cbr(false, "Denne karakter er banned: "..reason) end
        return
      end

      if config.whitelist and not userdata.whitelisted then
        if cbr then cbr(false, "Denne karakter er ikke whitelistet ["..user_id.."].") end
        return
      end

      if current_user_id ~= nil and current_user_id ~= user_id then
        clearUserSession(current_user_id, player)
      end

      startUserSession(player, user_id, idkey, GetPlayerName(player), function()
        if cbr then cbr(true, nil, user_id, created) end
      end)
    end)
  end)
end)
...
AddEventHandler("playerConnecting",function(name,setMessage, deferrals)
  ...
  local source = source
  local primary_identifier = vRP.getPrimaryIdentifier(source)
  local function acceptConnection()
    if isMulticharacterStarted() and primary_identifier ~= nil then
      multicharacter_active_identifiers[primary_identifier] = source
      multicharacter_source_identifiers[source] = primary_identifier
    end
...
AddEventHandler("playerDropped",function(reason)
  local source = source
  local primary_identifier = multicharacter_source_identifiers[source] or vRP.getPrimaryIdentifier(source)
  if primary_identifier ~= nil then
    multicharacter_active_identifiers[primary_identifier] = nil
    multicharacter_source_identifiers[source] = nil
  end
...
AddEventHandler("vRPcli:playerSpawned", function()
  ...
  if user_id ~= nil then
    ...
    SetTimeout(2000, function() -- trigger spawn event
      if vRP.getUserSource(user_id) ~= player or vRP.getUserDataTable(user_id) == nil then
        return
      end
