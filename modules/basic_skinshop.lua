      ...
      menudata.onclose = function(player)
        ...
        vRPclient.getCustomization(source,{},function(custom)
          ...
          if vRP.tryFullPayment(user_id,price) then
             if price > 0 then
               TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.money.paid({price})}, type = "success", queue = "global",timeout = 4000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            end
          else
              TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.money.not_enough()}, type = "error", queue = "global",timeout = 4000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            -- revert changes
