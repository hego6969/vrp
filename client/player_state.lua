...
local blockedPlayerModels = {
  [`player_zero`] = true,
  [`player_one`] = true,
  [`player_two`] = true,
}

local function normalizePlayerModel(model)
  if blockedPlayerModels[model] then
...
function tvRP.replaceWeapons(weapons)
  ...
  if oxInventoryStarted() then
    print("[vRP weapon debug] replaceWeapons ignored because ox_inventory is active")
    return old_weapons
  end
...
function tvRP.giveWeapons(weapons,clear_before)
  if oxInventoryStarted() then
    print("[vRP weapon debug] giveWeapons ignored because ox_inventory is active")
    return
  end
...
function tvRP.removeWeapons(weapons)
  if oxInventoryStarted() then
    print("[vRP weapon debug] removeWeapons ignored because ox_inventory is active")
    return
  end
...
function tvRP.setCustomization(custom) -- indexed [drawable,texture,palette] components or props (p0...) plus .modelhash or .model
  ...
  Citizen.CreateThread(function() -- new thread
    if custom then
      ...
      if mhash ~= nil then
        mhash = normalizePlayerModel(mhash)
